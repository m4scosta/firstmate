...
| `fm-cursor-cloud.sh`     | The cursor-cloud pane shim: launch a Cursor Cloud Agent, render its event stream, queue steers read from stdin, and cancel a still-active run |
| `fm-cursor-cloud-lib.sh` | Shared cursor-cloud run record, run-status vocabulary, status-line mapping, SSE frame decoding, and steer confirmation |
